...
class ReportLabPdfRendererAdapter(PdfRendererPort):
        ...
        *,
        ...
        try:
            ...
            raise InvalidInputError(
                user_message="Failed to render the PDF.", detail=str(exc)
            ) from exc

    def _validate_inputs(
        self, *, images: Sequence[ImageAsset], layout: LayoutPlan
    ) -> None:
        if len(images) not in (1, 2):
            raise InvalidInputError(user_message="Renderer requeires 1 or 2 images")
    ...
    def _draw_images(
        self, *, pdf: canvas.Canvas, images: Sequence[ImageAsset], layout: LayoutPlan
    ) -> None:
        for placed in layout.placed_images:
    ...
    def _draw_watermark(
        self, *, pdf: canvas.Canvas, layout: LayoutPlan, watermark: WatermarkSpec
    ) -> None:
    ...
    def _to_pil_image(asset: ImageAsset) -> Image:
        ...
        return Image.frombytes(asset.mode, (width, height), asset.pixel_bytes)
