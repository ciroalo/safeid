...
class CreateWatermarkedPdfUseCase:
    ...
    def execute(
        self, request: CreateWatermarkedPdfRequest
    ) -> CreateWatermarkedPdfResult:
        ...
        pdf_bytes = self.pdf_renderer.render_pdf_bytes(
            images=decoded, layout=layout, watermark=request.watermark
        )
    ...
    def _plan_layout(images: Sequence[ImageAsset]) -> LayoutPlan:
        ...
        policy = LayoutPolicy(allow_rotation=True, allow_scaling=True, min_gap_mm=12.0)
