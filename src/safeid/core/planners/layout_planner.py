...
def plan_a4_vertical_stack(
    *,
    ...
    return LayoutPlan(
        page=page, placed_images=[placed_top, placed_bottom], photo_area=photo_area
    )
...
def _place_single(
    ...
    *,
    ...
    for rotate_90 in rotations:
        ...
        fitted = _fit_single_candidate(
            w_px, h_px, avail_w, avail_h, allow_scaling=allow_scaling
        )
        if fitted is None:
            continue
    ...
    best_rotate, best_w, best_h, _ = max(
        candidates, key=lambda candidate: (candidate[3], -candidate[0])
    )
...
def _fit_single_candidate(
    w_px: int, h_px: int, avail_w: float, avail_h: float, *, allow_scaling: bool
...
def _place_two(
    ...
    *,
    ...
    candidates: list[
        tuple[float, int, PlacedImage, PlacedImage]  # total area  # total rotations
    ] = []
